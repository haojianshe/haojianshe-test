...
class ThumbUploadAction extends MBaseAction
{
	...
	private $ossobject = 'cms/posid/startpage';
    ...
    public function run()
    {
    	...
    	if(!$request->isPost){   
    		...
    		//检查图片大小和类型
    		if($file['size']>512000){
    			return $this->controller->render('thumbupload', ['model' => $model,'msg'=>'图片不能大于500k']);
    		}
    		...
    		//图片json信息
    		$hwtmp = AliOssService::getFileHW($model['url']);
            //判断是否取到宽高
            if(!$hwtmp['height'] || !$hwtmp['width']){
                return $this->controller->outputMessage(['errno'=>1,'msg'=>$file['name'].'上传失败']);
            }
    		$imginfo['h'] = $hwtmp['height'];
    		$imginfo['w'] = $hwtmp['width'];
    		$imginfo['url'] = $model['url'];
    		$model['imginfo'] =json_encode($imginfo);
