...
class EditAction extends MBaseAction {
    ...
    private function postHandle() {
        ...
        //先获取model
        $model = new TurntablePrizeService();
        if ($request->post('isedit') == 1) {
            $prizesid = $request->post('prizesid');
            $model = TurntablePrizeService::findOne(['prizesid' => $prizesid]);
        } else {
            $prizesid = 0;
        }
        ...
        if ($request->post('type') == 1 || $request->post('type') == 2|| $request->post('type') == 4) {
    ...
    private function getRetModel($prizesid) {
        if ($prizesid == 0) {
            ...
            $newmodel = TurntablePrizeService::findOne(['prizesid' => $prizesid]);
            $ret['model'] = $newmodel;
        }
        return $ret;
    }
