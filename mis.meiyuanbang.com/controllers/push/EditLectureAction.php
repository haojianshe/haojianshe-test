 ...
 * 文章添加和修改页面
 */
class EditLectureAction extends MBaseAction
{   
    ...
    { 
        ...
        if(!$request->isPost){
            ...
            if($id){
                ...
                return $this->controller->render('editlecture', ['model' => $model,'msg'=>$msg]);
            }
            else{
                //add
            ...
            $device_open_detail='lecture'; //app跳转识别跳转
            $model = new MisXingePushService();
            ...
            $model->push_person=$request->post('MisXingePushService')['push_person'];
            $model->send_time=strtotime($request->post('MisXingePushService')['send_time']);
            $model->create_time=time();
            $model->state=$state;
            $model->device_open_detail=$device_open_detail;
            $model->url_params='meiyuan://'.$device_open_detail.'?url='.urlencode($request->post('MisXingePushService')['url_params']);
            ...
            return $this->controller->render('editlecture', ['model' => $model,'msg'=>$msg,'isclose'=>$isclose]);
        }
    }
}
