 ...
 * 直播推送
 */
class EditLiveAction extends MBaseAction
{   
    ...
    { 
        ...
        if(!$request->isPost){
            ...
            if($id){
                ...
                return $this->controller->render('editlive', ['model' => $model,'msg'=>$msg]);
            }
            else{
                //add
            ...
            $device_open_detail='live'; //app跳转识别跳转
            $model = new MisXingePushService();
            ...
            $model->url_params='meiyuan://'.$device_open_detail.'?'.'url='.urlencode($request->post('MisXingePushService')['url_params'])."&liveid=".$request->post('MisXingePushService')['url_params_id'];
            ...
            return $this->controller->render('editlive', ['model' => $model,'msg'=>$msg,'isclose'=>$isclose]);
        }
    }
}
