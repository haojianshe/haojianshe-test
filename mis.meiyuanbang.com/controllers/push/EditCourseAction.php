 ...
 * 课程推送
 */
class EditCourseAction extends MBaseAction
{   
    ...
    { 
        ...
        if(!$request->isPost){
            ...
            if($id){
                ...
                return $this->controller->render('editcourse', ['model' => $model,'msg'=>$msg]);
            }
            else{
                //add
            ...
            $device_open_detail='course'; //app跳转识别跳转
            $model = new MisXingePushService();
            ...
            $model->url_params='meiyuan://'.$device_open_detail.'?'.'courseid='.urlencode($request->post('MisXingePushService')['url_params']);
            ...
            return $this->controller->render('editcourse', ['model' => $model,'msg'=>$msg,'isclose'=>$isclose]);
        }
    }
}
