 ...
 * 个人主页修改
 */
class EditHomeAction extends MBaseAction
{   
    ...
    { 
        ...
        if(!$request->isPost){
            ...
            if($id){
                ...
                return $this->controller->render('edithome', ['model' => $model,'msg'=>$msg]);
            }
            else{
            ...
            $device_open_detail='home'; //app跳转识别跳转
            $model = new MisXingePushService();
            ...
            $model->url_params='meiyuan://'.$device_open_detail.'?'.'uid='.urlencode($request->post('MisXingePushService')['url_params']);
            ...
            return $this->controller->render('edithome', ['model' => $model,'msg'=>$msg,'isclose'=>$isclose]);
        }
    }
}
