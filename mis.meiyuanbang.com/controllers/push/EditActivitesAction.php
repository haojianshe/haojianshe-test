 ...
 * 活动修改
 */
class EditActivitesAction extends MBaseAction
{   
    ...
    { 
        ...
        if(!$request->isPost){
            ...
            if($id){
                ...
                return $this->controller->render('editactivites', ['model' => $model,'msg'=>$msg]);
            }
            else{
            ...
            $device_open_detail='activities'; //app跳转识别跳转
            $model = new MisXingePushService();
            ...
            $model->url_params='meiyuan://'.$device_open_detail.'?'.'url='.urlencode($request->post('MisXingePushService')['url_params_url']).'&activityid='.$request->post('MisXingePushService')['url_params_id'];
            ...
            return $this->controller->render('editactivites', ['model' => $model,'msg'=>$msg,'isclose'=>$isclose]);
        }
    }
}
