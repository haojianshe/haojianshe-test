...
/**
 * 推送帖子搜索  meiyuan://search?tags=单线,男,单人,站立&type=1&wd=速写 人物速写
 */
class EditSearchAction extends MBaseAction
{   
    ...
    { 
        ...
        if(!$request->isPost){
            ...
            if($id){
                ...
                return $this->controller->render('editsearch', ['model' => $model,'msg'=>$msg,'catalog'=>$config]);
            }
            else{
            ...
            $device_open_detail='search'; //app跳转识别跳转
            $model = new MisXingePushService();
            ...
            $model->url_params='meiyuan://'.$device_open_detail.'?type=1&tags='.$request->post('MisXingePushService')['tags'].'&wd='.$request->post('MisXingePushService')['f_catalog'].' '.$request->post('MisXingePushService')['s_catalog'];
            $model->device_open_type=$device_open_type;
            $model->push_device=$request->post('MisXingePushService')['push_device'];
            //判断分类是否为空
            if(empty($request->post('MisXingePushService')['tags']) || empty($request->post('MisXingePushService')['s_catalog'])|| empty($request->post('MisXingePushService')['f_catalog'])){
                $msg ='推送分类 或标签不能为空';
                $isclose = true;
                return $this->controller->render('editsearch', ['model' => $model,'msg'=>$msg,'isclose'=>$isclose,'catalog'=>$config]);
            }
            //群发状态判断时间
