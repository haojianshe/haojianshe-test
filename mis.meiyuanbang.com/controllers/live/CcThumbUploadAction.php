 ...
 * 精讲添加和修改页面
 */
class CcThumbUploadAction extends MBaseAction
{
    ...
    public function run()
    {
    	...
    	if(!$request->isPost){   
    		...
    		$model['url'] = $url;
    		return $this->controller->render('ccthumbupload', ['model' => $model,'thumb'=>$thumb]);
    	}
    	else{
    		...
    		//检查图片大小和类型
    		if($file['size']>512000){
    			return $this->controller->render('ccthumbupload', ['model' => $model,'msg'=>'缩略图不能大于500k','thumb'=>$thumb]);
    		}
    		$fileext = AliOssService::getFileExt($file['name']) ; 
    		if(!in_array( $fileext, [".png", ".jpg", ".jpeg", ".gif", ".bmp"]))
    		{
    			return $this->controller->render('ccthumbupload', ['model' => $model,'msg'=>'图片格式错误','thumb'=>$thumb]);
    		}
    		//上传图片
    		$filename = AliOssService::getFileName($fileext);
    		$ret = AliOssService::picUpload($this->ossobject, $filename, $file);
    		if ($ret == false) {
   				return $this->controller->render('ccthumbupload', ['model' => $model,'msg'=>'上传图片失败','thumb'=>$thumb]);
    		}
    		//上传成功
    		$model['url'] = Yii::$app->params['ossurl'] . $ret;
    		return $this->controller->render('ccthumbupload', ['model' => $model,'msg'=>'上传成功','isclose'=>true,'thumb'=>$thumb]);
    	}
    }
}
