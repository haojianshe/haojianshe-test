...
class EditarticleAction extends MBaseAction {
    ...
    public function run() {
        ...
        return $this->controller->render('editarticle', $ret);
    }
    ...
    private function postHandle() {
        ...
        if ($request->post('isedit') == 1) {
            ...
            $articleModel = MybActivityArticleService::findOne(['newsid' => $newsid]);
        } else {
            $newsid = 0;
        }
        ...
        if ($newModel->save(true)) {
            ...
            if ($request->post('isedit') == 1) {
                $redis = Yii::$app->cache;
                $rediskey = "activity_article_" . $newModel->attributes['newsid'];
