 ...
 <form id="cmsform" action="/activity/cthumbupload?imgclass=<?= $thumb['imgclass'] ?>&valclass=<?= $thumb['valclass'] ?>" method="post" role="form" enctype="multipart/form-data">
 <table>
 	<tbody>
  ...
  <script>
        ...
        $('#adel').click(function(){
        	...
        	parent.$("#<?= $thumb['imgclass'] ?>").attr("src",newurl+'?d='+ Date.parse(new Date()));
       		parent.$('#<?= $thumb['valclass'] ?>').val('');
        	parent.layer.close(index);
        });
        <? } ?>
        ...
        //保存成功后更新父页面的对应图片，自动关闭
       	<?if(isset($isclose) && $isclose==true){ ?>
   			...
   			parent.$("#<?= $thumb['imgclass'] ?>").attr("src",newurl+'?d='+ Date.parse(new Date()));
       		parent.$('#<?= $thumb['valclass'] ?>').val(newurl);
       		parent.layer.close(index);
	   	<? }?>
	   	<?if(isset($msg) && $msg<>''){ ?>
