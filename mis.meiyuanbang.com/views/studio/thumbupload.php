 ...
 <form id="cmsform" action="/studio/thumbopus" method="post" role="form" enctype="multipart/form-data">
 <table>
 	<tbody>
  ...
  <script>
        ...
        $('#adel').click(function(){
        	...
        	parent.$("#imgthumb_<?= $model['name']?>").attr("src",newurl+'?d='+ Date.parse(new Date()));
       		parent.$('#thumb_<?= $model['name']?>').val('');
       		//parent.$('#thumb_rid').val(<?= $model['rcid']?>);
        	parent.layer.close(index);
        });
        <? } ?>
        ...
        //保存成功后更新父页面的对应图片，自动关闭
   		...
   		parent.$("#imgthumb_thumb_url").attr("src",newurl+'?d='+ Date.parse(new Date()));
       		parent.$('#thumb_thumb_url').val(newurl);
       		//parent.$('#imgthumb_thumb_url').attr('src',newurl);
       		parent.layer.close(index);
	   	<? }?>
	   	<?if(isset($msg) && $msg<>''){ ?>
