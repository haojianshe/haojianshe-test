...
<table cellspacing="0" cellpadding="0" class="content_list">
    ...
    <thead>
        <tr class="operate">
            ...
            <th colspan="2" style='text-align:right;'>
                <input type="button" onclick="addedit(0);" id="btnnew" value="新建问答" class="button"/>
            </th>
        </tr>
        <tr class="tb_header">
            <th >文章编号</th>
    ...
    <tr class="tb_list">
        ...
        <td><?= $model['answer_uids'] ?></td>
        <td><?= $model['username'] ?></td>
        <td><?= date('Y-m-d H:i', $model['ctime']) ?></td>
        <td>
            <a href='<?= Yii::$app->params['msiteurl'] . 'activity/qa_detail?newsid=' . $model['newsid'] ?>' target='_blank'>预览</a>
             <a name='aedit' newsid="<?= $model['newsid'] ?>" href='#'>编辑</a>
            <a name='adel' newsid="<?= $model['newsid'] ?>" href='#'>删除</a>
            <a name="qacomment" newsid="<?= $model['newsid'] ?>" href="#">问答</a>
        </td>
    </tr>
    <?}?>
    <!-- 分页 -->
...
<script type="text/javascript">
    ...
    function addedit(newsid) {
        var content = '/lkactivity/editquestions';
        var title = '添加问答';
        if (newsid > 0) {
            content = content + '?newsid=' + newsid;
            title = '编辑问答--编号:' + newsid;
        }
        layer.open({
            type: 2,
            title: title,
   ...
   //删除能力素材
    function del(newsid, status) {
     ...
     //问答
    $("a[name=qacomment]").click(function () {
        qacomment($(this).attr("newsid"));
        return false;
    });
   //获取评论列表
    function qacomment(newsid) {
        var content = '/lkactivity/qacomment?newsid=' + newsid;
        var title = '问答详情';
        layer.open({
            type: 2,
            title: title,
