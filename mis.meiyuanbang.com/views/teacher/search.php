  ...
  ?>   
   <table cellspacing="0" cellpadding="0" class="content_list">
    <thead>
      <tr class="operate">
        <th colspan="5" >
        	...
        	<select id="keytype" name="keytype" class="valid" value="1">
        	...
        	<input class="inputclass1" name="keyword" style="width:100px" type="text" value="<?=$keyword ?>" datatype="*1-20" nullmsg="请输入搜索条件！" sucmsg="&nbsp;" />
    ...
    <? foreach ($models as $model) { ?>
      ...
      <td>
      	<? if($addtype==0) { ?>
      		<a name='aadd' uid='<?= $model['uid'] ?>' href='#'>设为认证老师</a>  
      	<? } else{ ?>
      		<a name='aaddred' uid='<?= $model['uid'] ?>' href='#'>设为红笔老师</a> 
      	<? } ?>      	
      </td>
      </tr>  
     <?}?>
...
//设为红笔老师事件
$("a[name=aaddred]").on('click', function(){
	var userid = $(this).attr("uid");
	var content = '/teacher/rededit?from=search&userid='+userid;
	var title = '设置红笔老师';
	layer.open({
        type: 2,
        title: title,
