 ...
 *
 */
class AlipayNotifyAction extends ApiBaseAction{
    public  function run(){
        //log
        ...
        //对于微信或支付宝支付，用mark字段用来标记支付来源：browser等
        if($_POST['passback_params']){
        	$model->mark = $_POST['passback_params'];
        }
        else{
        	//app支付时通知时不添加passback_params字段
        	$model->mark = 'APP';
        }
        if($model->save()){   
            //改变订单状态
