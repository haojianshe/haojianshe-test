 ...
 * 
 */
class DkUpdateTeacherAction extends ApiBaseAction {
    ...
    private function _setForCorrect($activityData) {
        if (!empty($activityData)) {
            $teacherData = UserDetailService::find()->select('uid')->where(['featureflag' => 1])->asArray()->all();
            $newActivityArray = [];
            ...
            if (!empty($NewArray)) {
                foreach ($activityData as $key => $val) {
                    ...
                    foreach ($dkCorrect as $k1 => $v1) {
                        $randArray = rand(0, count($NewArray) - 1);
                        $dkCorrect[$k1]['teacheruid'] = $NewArray[$randArray];
                    }
